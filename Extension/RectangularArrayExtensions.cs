...
{
    public static int Columns<T>(this T[,] source)
    {
        return source.GetLength(0);
    }

    public static int Rows<T>(this T[,] source)
    {
    ...
    public static List<List<T>> ReversedRowsListOfLists<T>(this T[,] source)
    {
        var rows = source.Columns();
        var columns = source.Rows();
        ...
        {
            var tmp = new List<T>();
            for (var column = 0; column < columns; column++) tmp.Add(source[rows - row - 1, column]);
