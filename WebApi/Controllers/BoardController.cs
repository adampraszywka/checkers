...
public class BoardController(GameRepository gameRepository, BoardRepository boardRepository) : Controller
{
    ...
    public async Task<IActionResult> GetBoard([FromRoute] string gameId,
        [FromHeader(Name = HeaderPlayer.HeaderName)] string playerId)
    {
        ...
        if (!participation.DoesParticipate) return Forbid();

        var board = await boardRepository.Get(game.BoardId);
        if (board is null) throw new NotImplementedException();
    ...
    public async Task<IActionResult> GetPossibleMoves([FromRoute] string gameId, [FromRoute] int row,
        [FromRoute] int column, [FromHeader(Name = HeaderPlayer.HeaderName)] string playerId)
    {
        var game = await gameRepository.Get(gameId);
        if (game is null) return NotFound("Game not found");
        ...
        var player = new HeaderPlayer(playerId);
        var participation = game.Participation(player);
        if (!participation.DoesParticipate) return StatusCode(403);
        ...
        if (piece is null) throw new NotImplementedException();

        if (!participation.Participant.CanMove(piece)) return StatusCode(403);
    ...
    public async Task<IActionResult> MovePiece([FromRoute] string gameId, [FromBody] MoveDto request,
        [FromHeader(Name = HeaderPlayer.HeaderName)] string playerId)
    {
        var game = await gameRepository.Get(gameId);
        if (game is null) return NotFound("Game not found");
